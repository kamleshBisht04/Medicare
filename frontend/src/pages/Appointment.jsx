    ...
    docInfo && (
      <>
        ...
        {docInfo.isAvailable ? (
                ...
                docSlots[slotIndex]?.map((item, index) => (
                  <button
                    ...
                    className={`rounded-xl border px-4 py-3 text-sm font-medium transition-all duration-300 ${
                      item.time === slotTime
                        ? "border-primary bg-primary text-white shadow-md"
                        : "hover:border-primary hover:bg-primary/5 hover:text-primary border-gray-200 bg-white text-gray-600"
                    }`}
                  >
          ...
          <p className="text-md mt-28 px-4 text-center font-medium text-red-500 sm:pl-20">
            Doctor is currently unavailable. Please check another doctor or try
            again later.
          </p>
        )}
